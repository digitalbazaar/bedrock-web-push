...
api.vapidKeys.add = function(actor, vapidKey, options, callback) {
  ...
  async.auto({
    ...
    insert: ['checkPermission', (results, callback) => {
      logger.debug('[bedrock-web-push] adding VAPID key', vapidKey);
 ...
 *          [content] the content for the message (NOT PRESENTLY SUPPORTED).
 * @param callback(err) called once the operation completes.
 */
api.subscriptions.send = function(actor, subscriptionId, options, callback) {
  ...
  async.auto({
    ...
    send: ['getVapidKey', (results, callback) => {
      ...
      const headers = createVapidHeaders(subscription, vapidKey, email);
      headers.TTL = ('ttl' in options) ? options.ttl : DEFAULT_PUSH_MESSAGE_TTL;
      headers['Content-Length'] = 0;

      request.post(subscription.pushToken.endpoint, {
        headers: headers,
...
api.subscriptions.sendAll = function(
  ...
  async.auto({
    getSubscriptions: callback => api.subscriptions.getAll(null, {
      ...
      fields: {'subscription.id': 1}
    }, callback),
    send: ['getSubscriptions', (results, callback) => {
      const manifest = {};
      async.each(results.getSubscriptions, (subscription, callback) => {
        api.subscriptions.send(actor, subscription.id, options, err => {
          if(err) {
            manifest[subscription.id] = {
              result: 'error',
              error: err
            };
            if(options.removeBadSubscriptions &&
              ...
              return api.subscriptions.remove(
                actor, subscription.id, err => callback());
            }
          } else {
            manifest[subscription.id] = {
              result: 'success'
...
function createVapidHeaders(subscription, vapidKey, email) {
  const parsedUrl = Url.parse(subscription.pushToken.endpoint);
  ...
  return webpush.getVapidHeaders(
    audience, subject,
    vapidKey.publicKeyBase64Url, vapidKey.privateKeyBase64Url,
    TIME_IN_24_HOURS);
}
